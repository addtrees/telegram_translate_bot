...
sess_itc = onnxruntime.InferenceSession('seq2seq_itc.onnx') #для машинного перевода
sess_frc = onnxruntime.InferenceSession('seq2seq_frc.onnx') #для машинного перевода
...
inputs = sess_itc.get_inputs()
#ниже указать ранее сгенерированный bot_father'ом токен вашего бота
bot = telebot.TeleBot('6220831153:AAHH0pB-v2dmbw7fZHTa9wBUReGEp-kxQe4')
...
def Translate(message):
	...
	enc_src = sess_itc.run(None, {'src_tensor': src_tensor,
								  'src_mask': src_mask})[0]
	trg_indexes = [TRG_STOI[TRG_SOS]]
	for i in range(128):
		trg_tensor = np.int64(trg_indexes).reshape(1, -1)
		trg_mask = get_trg_mask(trg_tensor)
		output, attention = sess_frc.run(None, {'trg_tensor': trg_tensor,
												'enc_src': enc_src,
												'trg_mask': trg_mask,
												'src_mask': src_mask})
...
def get_text_messages(message):
	...
	if flag == 0:
		if message.text == "/start":
			...
			bot.send_message(message.from_user.id, "Напиши текст на Итальянском языке для перевода на Французский")
			flag = 2
		else:
